...
router.get('/', async(req, res) => {
    ...
    const allGeneres = await Genres.find()
    const films = await Film.find(options).limit(limit).skip(page * limit).populate('genre').populate('country')
    const user = req.user ? await User.findById(req.user._id) : {}
    res.render('index', {genres: allGeneres, user, films, pages: Math.ceil(totalFilms / limit)} )
})
...
router.get("/profile/:id", async(req, res) => {
    ...
    if(user){
        res.render("profile", {genres: allGeneres, user: user, loginUser: req.user} )
    }else{
...
router.get("/admin/:id", async(req, res) => {
    ...
    res.render("adminProfile", {genres: allGeneres, loginUser: req.user ? req.user : {}, user: user, films: films})
})

router.get("/new", async(req, res) => {
    const allGeneres = await Genres.find()
    const allCountries = await Country.find()
    res.render("newFilm", {genres: allGeneres, countries: allCountries, user: req.user ? req.user : {}})
})
...
router.get("/edit/:id", async(req, res) => {
    ...
    res.render("editFilm", {genres: allGeneres, countries: allCountries, user: req.user ? req.user : {}, film: film})
})
...
router.get('/detail/:id', async(req, res) => {
    ...
    res.render('detail', {user: req.user ? req.user : {}, film: film, film: film, rates: rates, averageRate: (averageRate / rates.length).toFixed(1)})
})
